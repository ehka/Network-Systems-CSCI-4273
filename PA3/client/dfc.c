...
int put_func(char * file_name, int sock[]){
	...
	int block_size_last = file_size - 3*block_size;
	printf("%d %d %d\n", file_size, block_size, block_size_last );
	for (int i=0; i < 4; i++)
		if (i == 3){
					part[i] = calloc(block_size_last, sizeof(char));
				}
		else
					part[i] = calloc(block_size, sizeof(char));

	int j = 0;
	MD5_Init (&mdContext);
	int ssize = block_size;
	if(fp != NULL){
	while((n_bytes = fread(part[j], 1, ssize,fp)) > 0){
			printf("%d\n",n_bytes );
			MD5_Update(&mdContext,part[j], n_bytes);
			j++;
			if(j == 3){
				ssize = block_size_last;
			}
			if(j == 4){
				break;
			}
